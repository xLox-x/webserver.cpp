...
void Channel::HandleEvent() const{
    if(tied_){
        ...
        std::cout << ">>Channel::HandleEvent use_count:" << guard.use_count() << std::endl;
        HandleEventWithGuard();
        std::cout << ">>Channel::HandleEvent end:" << std::endl;
    }else{
        HandleEventWithGuard();
    }
}
...
void Channel::Tie(const std::shared_ptr<void> &ptr){
    tied_ = true;
    tie_ = ptr;
    std::cout << ">>Channel::Tie use_count:" << ptr.use_count() << std::endl;
}
